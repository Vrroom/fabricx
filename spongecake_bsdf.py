...
class SurfaceBased_sampleSGGX (mi.BSDF) : 
    ...
    def sample(self, ctx, si, sample1, sample2, active):
        ...
        G = dr.select(selected_r, G_r, G_t)
        G = dr.select(dr.isfinite(G), G, mi.Float(0.0))
    ...
    def eval_pdf(self, ctx, si, wo, active):
        ...
        D = sggx_pdf(h, S)        
        D = dr.select(dr.isfinite(D), D, 0.0)
        pdf_spec = D / (4. * sggx_projected_area(si.wi, S))
    ...
    def to_string(self):
        return ('SurfaceBased_sampleSGGX[]')
...
class SurfaceBased_sampleDiffuse (mi.BSDF) : 
    ...
    def to_string(self):
        return ('SurfaceBased_sampleDiffuse[]')
...
class SurfaceBased_sampleBoth (mi.BSDF) : 
    ...
    def eval_pdf(self, ctx, si, wo, active):
        ...
        pdf = (self.specular_prob * pdf_spec + (1-self.specular_prob) * pdf_diffuse) * delta_val
        f_sponge_cake = (color * D * G) / (4. * dr.abs(cos_theta_i))
    ...
    def to_string(self):
        return ('SurfaceBased_sampleBoth[]')
