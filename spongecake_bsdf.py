...
def sample_diffuse (pcg, sample2, wi, S) : 
    ...
    D = sggx_pdf(h, S)
    sample_iid = mi.Point2f(pcg.next_float32(), pcg.next_float32())
    wo = mi.warp.square_to_cosine_hemisphere(sample_iid) 
    pdf = mi.warp.square_to_cosine_hemisphere_pdf(wo)
    return h, D, wo, pdf

def pdf_specular(wi, wo, S):
    h = dr.normalize(wi + wo)
    D = sggx_pdf(h, S)
    pdf = D / (4. * sggx_projected_area(wi, S))
    return D, pdf

def pdf_diffuse(wi, wo, S):
    h = dr.normalize(wi + wo)
    D = sggx_pdf(h, S)
    pdf = dr.dot(wo, h) / dr.pi
    return D, pdf
...
class SurfaceBased (mi.BSDF) : 
    ...
    def __init__ (self, props, *args, feature_map_type=None, cloth_type=None, texture=None, perturb_specular=False, delta_transmission=False) : 
        ...
        diffuseR_flags = mi.BSDFFlags.DiffuseReflection | mi.BSDFFlags.FrontSide | mi.BSDFFlags.BackSide        
        reflection_flags = mi.BSDFFlags.GlossyReflection | mi.BSDFFlags.FrontSide | mi.BSDFFlags.BackSide
        diffuseT_flags = mi.BSDFFlags.DiffuseTransmission | mi.BSDFFlags.FrontSide | mi.BSDFFlags.BackSide
        ...
        self.m_components = [diffuseR_flags, reflection_flags, diffuseT_flags, transmission_flags, delta_transmission_flags]
        self.m_flags = diffuseR_flags | reflection_flags | diffuseT_flags | transmission_flags | delta_transmission_flags
    ...
    #     # # Mandy turns off meso for now
    #     # f_surface_based_meso = f_surface_based_micro
    #     f_surface_based_meso = f_diffuse
    ...
    def sample(self, ctx, si, sample1, sample2, active): 
        ...
        F = color + (1.0 - color) * ((1 - dr.abs(dr.dot(h, wo_diffuse))) ** 5)
        ...
        cos_theta_o = cos_theta_o_diffuse
        ...
        active = active & (cos_theta_i * cos_theta_o > 0) & (pdf_diffuse > 0.0)
        
        weight = f_surface_based_meso * dr.abs(cos_theta_o) / pdf_diffuse
        weight = dr.select(active, weight, mi.Color3f(0.0,0.0,0.0))      
    ...
    def eval_pdf(self, ctx, si, wo, active):
        ...
        # v_threshold = 0.5        
        #pdf_specular = D / (4. * sggx_projected_area(si.wi, S))
        #pdf = self.specular_prob * pdf_specular + (1-self.specular_prob) * pdf_diffuse
        ...
        # Meso-scale BSDF
        f_surface_based_meso = f_surface_based_micro * V_i * V_o
        value =  f_surface_based_meso * cos_theta_o
    ...
    #     h = dr.normalize(si.wi + wo)
    #     D, pdf = pdf_specular(si.wi, wo, S)
    #     D_, pdf_ = pdf_diffuse(si.wi, wo, S)
    #     D = self.specular_prob * D + (1-self.specular_prob) * D_
    #     # pdf = self.specular_prob * pdf + (1-self.specular_prob) * pdf_
    #     pdf = 1.0

    #     # # delta transmission (assume no delta transmission for now)
    #     # h = dr.select(selected_dt, h_dt, h)
    #     # D = dr.select(selected_dt, D_dt, D)
    #     # wo = dr.select(selected_dt, wo_dt, wo)
    #     # pdf = dr.select(selected_dt, pdf_dt, pdf)
    ...
    #     # v_threshold = 0.5
    #     active = active & dr.neq(cos_theta_i, 0.0) & dr.neq(D, 0.0) & dr.neq(dr.dot(wo, h), 0.0) # & (V_i > v_threshold) & (V_o > v_threshold)
    #     # active =  active & (cos_theta_i != 0.0 ) & (D != 0.0) & (dr.dot(wo, h) != 0.0)
    ...
    #         (1.0 - self.w)
    #     )
    #     # dr.printf_async("f_diffuse.z = %f\n", f_diffuse.z, active=active)
    #     # exit()

    #     # dr.printf_async("f_sponge_cake.z = %f\n", f_sponge_cake.z, active=active)

    #     # # print("self.specular_prob", self.specular_prob)
    #     # f_surface_based_micro = self.specular_prob * f_sponge_cake + \
    #     #                         (1 - self.specular_prob) * f_diffuse
        
    #     f_surface_based_micro = (1 - self.specular_prob) * f_diffuse
    ...
    #     #dr.printf_async("f_overall.x = %f\n", f_overall.x, active=active)
    #     #dr.printf_async("f_overall.y = %f\n", f_overall.y, active=active)
    #     # dr.printf_async("f_overall.z = %f\n", f_overall.z, active=active)


    #     # f_overall = dr.select(active, f_overall, mi.Color3f(0.0, 0.0, 0.0))

    #     # f_overall = 0.0
    #     # pdf = 0.0

    #     # dr.printf_async("pdf = %f\n", pdf, active=active)

    #     return f_overall, pdf
